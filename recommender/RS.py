...
class RS(_RecommenderInit):
    ...
    def n_nearest_items(self, nr_of_items, mode, method, recommender, sim='cosine'):
        ...
        # Create dataframe
            df_products = pd.DataFrame(index.astype(int), columns=(['Product {}.'.format(s) for s in np.arange(1, nr_of_items + 1, 1)]))
            ...
            df_id = pd.DataFrame(index.astype(int),columns=(['id {}.'.format(s) for s in np.arange(1, nr_of_items + 1, 1)]))
    ...
    def single_product(self, product_name, nr_of_items, method, mode, recommender, sim='cosine'):
        ...
        for i in range(nr_of_items):
            print("{}: {} with a similarity rating of {} ".format((i + 1), df.iloc[item_id][i + 1], round(df.iloc[item_id][(df.shape[1] // 3)*2 + i + 2], 3)))
...
if __name__ == '__main__':
    ...
    rs.single_product(product_name="#2 Coffee Filters", nr_of_items=15, method='rating', mode='count', recommender='item')
